    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            List<ParsedFile> ParseCSV(string path, string seperator = ",", bool header = true, int skip = 0)
            {
                ...
                {
                    try {
                        string[] observations = row.Split(Convert.ToChar(seperator));
            ...
            DateTime[] Dates1 = parsed1.Select(r => r.Date).ToArray();
            DateTime[] Dates2 = parsed2.Select(r => r.Date).ToArray();
            ...
            if (perforUnitTests)
            {
                ...
                var trendingdata = Enumerable.Range(0, 1000).Select(i => (double)i);
                int slope = 10;
                int offsett = 50;
                DenseVector y = DenseVector.OfEnumerable(trendingdata.Select(i => i + offsett));
                DenseVector x = DenseVector.OfEnumerable(trendingdata.Select(i => i / slope));
